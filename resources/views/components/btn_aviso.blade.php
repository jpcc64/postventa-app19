...
<button
    onclick="document.getElementById('confirmationModal_{{ $clientes['ServiceCallID'] }}').classList.remove('hidden')"
...
<!-- Modal -->
<div id="confirmationModal_{{ $clientes['ServiceCallID'] }}"
        ...
        <div class="mt-3 text-center">
            ...
            <div class="mt-2 px-7 py-3">
                ...
                <p class="text-lg font-semibold text-gray-800 my-2">
                    ...
                    {{ $clientes['Telephone'] ?? 'No disponible' }}
                </p>
                <p class="text-sm text-gray-500">
                    Si desea enviarlo a otro número, por favor, introdúzcalo a continuación.
                </p>
                <form id="avisarForm_{{ $clientes['ServiceCallID'] }}"
                    action="{{ route('avisar', trim($clientes['ServiceCallID'])) }}" method="post" class="mt-4">
                    @csrf
                    <input type="text" name="telefono_alternativo" placeholder="Número de teléfono alternativo"
                        class="w-full px-3 py-2 text-gray-700 border rounded-lg focus:outline-none" />
                    <input type="hidden" name="telefono_original" value="{{ $clientes['Telephone'] ?? '' }}">
                </form>
            </div>
            <div class="items-center px-4 py-3">
                <button
                    onclick="document.getElementById('confirmationModal_{{ $clientes['ServiceCallID'] }}').classList.add('hidden')"
                ...
                <button onclick="document.getElementById('avisarForm_{{ $clientes['ServiceCallID'] }}').submit()"
