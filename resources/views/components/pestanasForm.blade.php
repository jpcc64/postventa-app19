    ...
    <div class="w-full col-span-3">
        <div class="mb-6 ">
            ...
            <div x-show="tab === 'anexo'" x-cloak x-transition class="space-y-4 mb-6">
                ...
                <div class="mt-4">
                    <button type="button" id="upload-button" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                        Subir Anexos
                    </button>
                </div>
        </div>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                const anexoTabButton = document.querySelector('button[ @click="tab = \'anexo\'"]');
                if(anexoTabButton){
                    anexoTabButton.addEventListener('click', function() {
                        loadAttachments();
                    });
                }
                ...
                function loadAttachments() {
                    const serviceCallID = document.querySelector('input[name="ServiceCallID"]').value;
                    if (!serviceCallID) {
                        return;
                    }

                    fetch(`/anexos/${serviceCallID}`)
                    ...
                    .then(data => {
                        ...
                        data.forEach(file => {
                            ...
                            if (file.name.match(/\.(jpeg|jpg|gif|png)$/i)) {
                                filePreview = `<img src="${file.url}" alt="${file.name}">`;
                            } else {
                                filePreview = `<div class="file-icon"><svg class="w-16 h-16 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0011.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg></div>`;
                            }
                            previewItem.innerHTML = `
                                ${filePreview}
                                <div class="file-info"><a href="${file.url}" target="_blank" class="file-name">${file.name}</a></div>
                            `;
                ...
                function uploadFiles() {
                    const serviceCallID = document.querySelector('input[name="ServiceCallID"]').value;
                    if (!serviceCallID) {
                        alert('Por favor, guarde el parte antes de subir anexos.');
                        return;
                    }
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            alert(data.error || 'Ocurrió un error al subir los archivos.');
                        }
                    })
                    .catch(error => {
