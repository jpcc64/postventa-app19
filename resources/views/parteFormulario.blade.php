    ...
    <div x-data="{ filter: '' }">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 max-w-5xl mx-auto">
            ...
            @foreach ($partes as $parte)
                <div class="col-span-1"
                    x-show="filter === '' || '{{ $parte['ServiceCallID'] }}'.toLowerCase().includes(filter.toLowerCase()) || '{{ $parte['CreationDate'] }}'.toLowerCase().includes(filter.toLowerCase()) || '{{ addslashes($parte['U_H8_Nombre']) }}'.toLowerCase().includes(filter.toLowerCase()) || '{{ addslashes($parte['ItemDescription']) }}'.toLowerCase().includes(filter.toLowerCase())">
